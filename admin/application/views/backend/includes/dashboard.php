               ...
               <div class="col-md-6" style="margin-top:10px;">
                    <section class="panel2">
                        <div id="formcategory" class="form-group">
                                ...
                                echo form_dropdown('category',$category,set_value('category'),'  onchange="changelistingdropdown()" id="select1" class="chzn-select form-control categoryvalue" 	data-placeholder="Choose a Category..."');
                            ?><br>
<!--                                <a class="btn btn-info categoryformsubmit">Enter</a>-->
                          </div>
                    </section>
                </div>
                <div class="col-md-6 formlisting showlistingdropdown" style="display:none;margin-top:10px;">
...
<!--                                <a class="btn btn-info listingformsubmit">Enter</a>-->
                          ...
                          <div class="col-sm-4 listingselect">
                       <select name="listing" class="chzn-select form-control" data-placeholder="Choose a Category..." id="select2" style="width:200px;">
...
//             $("#user .userdetails").append("<div>Name:"+firstname+"</div><div>Address:"+address+"</div><div>City:"+city+"</div><div>Contact:"+contact+"</div><div>DOB:"+dob+"</div><div>Email:"+email+"</div><div>Pincode:"+pincode+"</div>");
    ...
    function changelistingdropdown() {
//        console.log($('#cityid').val());
        alert($("#select1").val());
        $("#select2").select2("val", "");
        console.log( $("#select1").val());
        $.getJSON(
            "<?php echo base_url(); ?>index.php/site/getlistingbycategorydashboarddropdown/" + $("#select1").val(), {
                id: "123"
            },
            function (data) {
                console.log(data);
                nodata=data;
                changelistingdropdownafter(data);
    ...
    function changelistingdropdownafter(data) {
        $(".listingselect select").html("");
        console.log(data);
        for(var i=0;i<data.length;i++)
        {
//            console.log(data[i].id);
            $(".listingselect select").append("<option value='"+data[i].id+"'>"+data[i].name+"("+data[i].cityname+" "+data[i].areaname+")</option>");
